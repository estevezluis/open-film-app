			...
			{!play && <>
				<main className="bg-gray-900 h-screen">
						...
						<div className="relative w-full md:flex mb-12">
							...
							<div className="w-full md:max-w-[650px] mt-40 relative">
								...
								<div className="flex flex-col items-center md:items-start mb-8">
									<p className="font-whitney text-white text-center md:text-left transition-[max-height] duration-[600ms] ease-in-out max-h-[60px] line-clamp-2 font-medium" style={{ color: "rgb(245, 244, 243)" }} id="show-description">
										{film.description}
									</p>
								</div>

								<div className="flex flex-row flex-wrap justify-center md:justify-start gap-2 text-gray-500 md:text-gray-300 mb-9">
									<p className="font-whitney font-medium text-inherit">{film.releaseYear}</p>
									{getSpacer()}
									{film.categories.map((category, i) => {
										return (
											<>
												<p className="font-whitney font-medium text-inherit">{category}</p>
												{i !== film.categories.length - 1 && getSpacer()}
											</>
										)
									})}
									<div className="flex flex-row gap-2 md:basis-full">
										<label className={badgeClassName}>{film.rating}</label>
										<label className={badgeClassName}>HD</label>
										<label className={badgeClassName}>CC</label>
									</div>
								</div>
					...
					<div className='w-full relative bg-gray-900'>
						<div className="relative" style={{ transform: 'translate3d(0px, 0px, 0px)' }}>
									...
									<div className="relative flex flex-nowrap w-full">
										<div className="flex flex-nowrap w-full overflow-hidden relative before:content-[&quot;&quot;] before:block before:w-6 before:h-full before:absolute before:top-0 before:opacity-0 before:transition-opacity before:pointer-events-none before:z-10 after:content-[&quot;&quot;] after:block after:w-6 after:h-full after:absolute after:top-0 after:opacity-0 after:transition-opacity after:pointer-events-none after:z-10">
											<nav className="horizontal-scroller w-full">
												<div className="flex flex-row items-stretch" role="tablist" aria-orientation="horizontal">
													{film.type === 'Episode' &&
														<button className="department-tab text-left mr-8 focus-visible:!ring-transparent focus-visible:!ring-offset-transparent focus-visible:!outline-none whitespace-nowrap  " role="tab" name="watch" data-cy="watch-tab" id="headlessui-tabs-tab-3" type="button" aria-selected="false" >
															<p className="cursor-pointer text-lg leading-8 font-whitney m-0 p-0 -tracking-[0.01em] text-white/70">Watch</p>
														</button>
													}
													<button className="department-tab text-left mr-8 focus-visible:!ring-transparent focus-visible:!ring-offset-transparent focus-visible:!outline-none whitespace-nowrap  border-b-4 border-oxide selected" role="tab" name="about" data-cy="about-tab" id="headlessui-tabs-tab-4" type="button" aria-selected="true" aria-controls="headlessui-tabs-panel-8">
														<p className="cursor-pointer font-bold text-lg leading-8 font-whitney m-0 p-0 -tracking-[0.01em] text-white">More Details</p>
													</button>
												</div>
											</nav>
										</div>
