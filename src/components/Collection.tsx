...
export default function Collection({films} : {films: Film[]}) {
    ...
    const [movies, setMovies] = useState([templateMovie, templateMovie, templateMovie, templateMovie, templateMovie])
    const settings = {
        data: true,
        center: true,
        infinite: false,
                ...
                <section className="mb-12">
                    ...
                    <Slider {...settings}>
                        {movies.map(({ id, imageSource, title }, i) => {
                            return <div key={id === templateMovie.id ? i : id}>
                                {id === templateMovie.id ?
                                    <>
                                    ...
                                    <>
                                        <Link to={`/watch/${id}`}>
                                            <div className="relative rounded-lg cursor-pointer h-44 w-72 overflow-hidden duration-[400ms] ease-in-out mb-3">
                                                <img src={`${SERVER_URL}/${imageSource}`} alt={title} className="object-cover absolute inset-0 w-full h-full rounded-lg skeleton opacity-70" />
                                        ...
                                        <div className="w-72 overflow-hidden whitespace-nowrap text-ellipsis">
                                            <Link to={`/watch/${id}`} className="">{title}</Link>
                                        </div>
                                    </>
                                }
                            </div>
